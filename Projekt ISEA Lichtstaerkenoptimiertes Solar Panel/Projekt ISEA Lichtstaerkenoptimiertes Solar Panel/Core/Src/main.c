...
/*  irem verbesserungsversuche
uint32_t calcAbsDif(uint32_t LDR1, uint32_t LDR2) {

	uint32_t absolutDifferenz = abs(LDR1 - LDR2);
	return absolutDifferenz;
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
uint32_t LDR_1_value;
uint32_t LDR_2_value;
uint32_t SP_value;
